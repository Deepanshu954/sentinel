...
public class PredictionScheduler {
    ...
    public void executePredictionCycle() {
        ...
        if (featuresOpt.isEmpty()) {
            logger.error("InfluxDB unavailable or no recent data found. Skipping ML prediction cycle.");
            return;
        }

        Optional<PredictionResponse> predictionOpt = mlServiceClient.predict(featuresOpt.get());

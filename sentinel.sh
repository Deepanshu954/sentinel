...
case $1 in
  ...
  logs)
    ...
    docker-compose --env-file /dev/null logs -f $SERVICE
    ;;
  ...
  influx)
    ...
    docker-compose --env-file /dev/null exec influxdb influx user password \
      --name admin \
  ...
  build)
    ...
    docker-compose --env-file /dev/null build $SERVICE
    docker-compose --env-file /dev/null up -d $SERVICE
    echo -e "${GREEN}Done.${NC}"
    ;;
